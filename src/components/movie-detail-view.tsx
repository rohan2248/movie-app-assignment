...
function useDetailLayout() {
  ...
  const backdropHeight = Math.round(
    Math.min(
      contentWidth / BackdropAspectRatio,
      wide ? MAX_WIDE_BACKDROP_HEIGHT : Infinity,
    ),
  );
  ...
  return {
    ...
    posterOverlap: Math.round(
      (posterWidth / PosterAspectRatio) * (wide ? 0.45 : 0.4),
    ),
  };
}
...
export function MovieDetailView(props: Props) {
  ...
  return (
    <View
      style={{ width: "100%", maxWidth: MAX_DETAIL_WIDTH, alignSelf: "center" }}
    >
      <Backdrop
...
function MetaRow({
  ...
  if (base.rating !== null) {
    items.push(
      <View
        key="rating"
        style={{ flexDirection: "row", alignItems: "center", gap: Spacing.one }}
      >
      ...
      {failure && (
        <View style={{ gap: Spacing.two, alignItems: "flex-start" }}>
          <Banner
            tone="warning"
            icon="error"
            text={`${failure.title}. Showing what we already had.`}
          />
          {failure.canRetry && (
            <ActionButton label="Try again" onPress={onRetry} busy={retrying} />
          )}
        </View>
      )}
...
export function MovieDetailSkeleton() {
  ...
  return (
    ...
    >
      <Skeleton
        height={layout.backdropHeight}
        radius={layout.wide ? Radius.lg : 0}
      />
      <View
        style={{
